...
import { 
  ...
  Cell, PieChart, Pie, AreaChart, Area, CartesianGrid, Legend, ComposedChart, Line
} from 'recharts';
import { AnalyticsSummary, HeatmapPoint } from '../types';
import { APP_COLORS } from '../constants';
import { UserComparison } from '../services/dataProcessing';
...
export const DailyTrendChart: React.FC<{ summary: AnalyticsSummary; days?: number }> = ({ summary, days = 30 }) => {
  if (!summary.dailyTrend || summary.dailyTrend.length === 0) {
    return <div className="h-64 w-full flex items-center justify-center text-gray-500 text-xs">No daily trend data</div>;
  }

  // Filter to last N days
  const filteredData = summary.dailyTrend.slice(-days);
    ...
    <div className="h-64 w-full">
      <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
        <AreaChart data={formattedData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
          <defs>
            <linearGradient id="dailyPlaysGradient" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor={APP_COLORS.plexOrange} stopOpacity={0.4}/>
              <stop offset="95%" stopColor={APP_COLORS.plexOrange} stopOpacity={0}/>
            </linearGradient>
          ...
          <XAxis 
            ...
            axisLine={false}
            tickLine={false}
            interval={Math.floor(days / 7)} // Show ~7 labels
          />
          <YAxis 
            yAxisId="plays"
            orientation="left"
          ...
          <Tooltip 
            content={({ active, payload }) => {
              if (active && payload && payload.length) {
                ...
                return (
                  <div className="glass bg-[#15171C]/90 border border-white/10 p-3 rounded-xl shadow-2xl backdrop-blur-md z-50">
                    <p className="text-gray-400 font-medium text-xs mb-2">{data.date}</p>
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-[#e5a00d]"></div>
                        <p className="text-white text-sm font-bold">
                          {data.plays} <span className="text-gray-500 font-normal">plays</span>
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-blue-500"></div>
                        <p className="text-white text-sm font-bold">
                          {data.hours} <span className="text-gray-500 font-normal">hours</span>
                        </p>
                      </div>
                    </div>
                  </div>
...
export const UserComparisonChart: React.FC<{ users: UserComparison[] }> = ({ users }) => {
  if (!users || users.length === 0) {
    return <div className="h-64 w-full flex items-center justify-center text-gray-500 text-xs">No user data available</div>;
  }
          ...
          <Tooltip 
            content={({ active, payload }) => {
              if (active && payload && payload.length) {
                ...
                return (
                  <div className="glass bg-[#15171C]/90 border border-white/10 p-3 rounded-xl shadow-2xl backdrop-blur-md z-50">
                    <p className="text-gray-400 font-medium text-xs mb-2">{data.fullName}</p>
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-[#e5a00d]"></div>
                        <p className="text-white text-sm font-bold">
...
export const UserStatsTable: React.FC<{ users: UserComparison[] }> = ({ users }) => {
  if (!users || users.length === 0) {
    return <div className="text-center text-gray-500 py-8 text-sm">No user data available</div>;
  }

  return (
    <div className="space-y-3">
      {users.map((user, index) => (
        <div key={user.userName} className="flex items-center justify-between p-3 bg-white/5 rounded-xl hover:bg-white/10 transition-colors">
          <div className="flex items-center gap-4 flex-1">
            <span className={`
              w-8 h-8 flex-shrink-0 flex items-center justify-center rounded-lg text-sm font-bold shadow-inner
              ${index === 0 ? 'bg-[#e5a00d] text-black' : 'bg-gray-800 text-gray-400'}
            `}>
              {index + 1}
            </span>
            <div className="flex flex-col flex-1 min-w-0">
              ...
              <span className="text-xs text-gray-500">
                {user.topContent.length > 0 ? `Top: ${user.topContent[0].name}` : 'No content'}
              </span>
            </div>
          </div>
          <div className="flex items-center gap-6 text-sm">
            <div className="text-right">
              <div className="text-white font-bold">{user.totalHours}h</div>
              <div className="text-xs text-gray-500">Total</div>
            </div>
            <div className="text-right">
              <div className="text-white font-bold">{user.totalPlays}</div>
              <div className="text-xs text-gray-500">Plays</div>
            </div>
            <div className="text-right">
              <div className="text-white font-bold">{user.avgSessionMinutes}m</div>
